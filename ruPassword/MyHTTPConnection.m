...
- (NSString *)createBrowseableIndex:(NSString *)path
{
	...
	[outdata appendString:@"<html><meta http-equiv=\"Content-Type\" content=\"text/html;charset=UTF-8\"><head>"];
	[outdata appendFormat:@"<title>%@</title>",NSLocalizedString(@"KeePass Files", nil)];
	...
	[outdata appendFormat:@"<table id=\"keepass\"><thead><tr><th  colspan=2>%@</th></tr></thead><tbody>",NSLocalizedString(@"KeePass Files", nil)];
...
//        NSDictionary *fileDict = [[NSFileManager defaultManager] fileAttributesAtPath:[path stringByAppendingPathComponent:fname] traverseLink:NO];
		...
		if ([[fileDict objectForKey:NSFileType] isEqualToString: @"NSFileTypeRegular"]){
			[outdata appendFormat:@"<tr><td><a href=\"%@\">%@</a></td><td>(%8.1f %@)</td></tr>", fname, fname, [[fileDict objectForKey:NSFileSize] floatValue] / 1024, NSLocalizedString(@"Kb", nil)];
		}
	}
	...
	if ([self supportsMethod:@"POST" atPath:path])
	{
		...
		[outdata appendString:NSLocalizedString(@"upload file", nil)];
		[outdata appendString:@"<input type=\"file\" name=\"file\" id=\"file\" />"];
		[outdata appendString:@"</td><td>"];
		[outdata appendFormat:@"<input type=\"submit\" name=\"button\" id=\"button\" value=\"%@\" />",NSLocalizedString(@"Submit", nil)];
...
//	NSData *requestData = (__bridge NSData *)CFHTTPMessageCopySerializedMessage(request);
