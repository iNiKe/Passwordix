...
@synthesize ruPass, enPass, iMacSw, view1, ClipBtn, ruPassLabel, enPassLabel, iMacLabel, ruPassComment, md5TextView, sha1TextView, scrollView, transLabel, transTextView, navbar, rateButton;
...
-(void)updateHash
{
    md5TextView.text = [NSString md5:enPass.text];
    sha1TextView.text = [NSString sha1:enPass.text];
}
...
{
    if ( iMacSw.isOn ) 
        enPass.text = [NSString strReplaceFromTo:ruPass.text :rus1 : eng1];
    else
        enPass.text = [NSString strReplaceFromTo:ruPass.text :rus2 : eng2];
    
    transTextView.text = [NSString translitStr:ruPass.text];
...
{
    if ( iMacSw.isOn ) 
        ruPass.text = [NSString strReplaceFromTo:enPass.text :eng1 : rus1];
    else
        ruPass.text = [NSString strReplaceFromTo:enPass.text :eng2 : rus2];
    transTextView.text = [NSString translitStr:ruPass.text];
    [self checkCanClip];
    [self updateHash];
}
...
{
    BOOL editable;
    if ( (textField == md5TextView) || (textField == sha1TextView) || (textField == transTextView) )
    {
        editable = NO;
    }
    else
...
- (void)setupButtons
{
    ...
    [rateButton useImages:@"InstallButton.png" pressedImage:@"InstallButtonPressed.png" capWidth:3 capHeight:11];
}
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self)
    {
        ...
        self.tabBarItem.image = [UIImage imageWithContentsOfResolutionIndependentFile:@"converter.png"];
    }
    return self;
}
