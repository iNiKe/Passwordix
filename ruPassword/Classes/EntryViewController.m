...
{
	EditNodeViewController *editNodeViewController = [EditNodeViewController controllerWithDelegate:self];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if([indexPath section]==0){	
		...
		if (cell == nil) {
			cell = [[MultiLineTableViewCellStyle2 alloc] init:CellIdentifier];
		}		
		...
		if (cell == nil) {
			cell = [[TKTextViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier];
		}
	...
	if(!self.editing&&[indexPath section]==0){		
		if([indexPath row]!=WEBSITE_ROW){
			...
			UIActionSheet *actionSheet = [[UIActionSheet alloc] initWithTitle:nil
										  ...
										  nil];			
			[actionSheet showInView:[MyAppDelegate window]]; // show from our table view (pops up in the middle of the table)
		}
	}
}
